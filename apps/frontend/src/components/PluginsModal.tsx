  ...
  const pluginsCatalog: PluginDef[] = useMemo(() => [
      ...
      renderSettings: (_boardStore, onRun) => {
        return (
          <div className="space-y-4">
            <label className="text-[10px] uppercase font-bold text-[#9AA4AB]">Mermaid Code Input</label>
            <textarea 
              defaultValue="graph TD&#10;  A[Start] --> B(Process)&#10;  B --> C{Decision}&#10;  C -->|Yes| D[Done]"
              id="mermaid-code-textarea"
              className="w-full h-32 p-3 rounded-xl border border-[#E2E5E9] bg-white text-[#1A1D21] text-xs font-mono focus:outline-none"
            />
            <button
              onClick={() => {
              ...
              className="w-full py-2 bg-teal-650 hover:bg-teal-600 text-[#1A1D21] rounded-xl text-xs font-bold transition-all"
            >
              Generate Diagram
            </button>
          </div>
    ...
    },
    {
      ...
      renderSettings: (_boardStore, onRun) => {
        return (
          <div className="space-y-4">
            <label className="text-[10px] uppercase font-bold text-[#9AA4AB]">QR Target URL</label>
            <input 
              type="text" 
              defaultValue="https://collabboard.com" 
              id="qr-url-input"
              className="w-full px-3 py-2 rounded-xl border border-[#E2E5E9] bg-white text-[#1A1D21] text-xs focus:outline-none"
            />
            <button
              onClick={() => {
    ...
    },
    {
      ...
      renderSettings: (_boardStore, onRun) => {
        ...
        return (
          <div className="space-y-4">
            <label className="text-[10px] uppercase font-bold text-[#9AA4AB]">Choose Emoji</label>
            <div className="grid grid-cols-4 gap-2">
              {emojis.map((emoji) => (
                <button
                  key={emoji}
                  onClick={() => onRun({ emoji })}
                  className="py-2.5 bg-[#E2E5E9] hover:bg-slate-700 text-lg rounded-xl transition-all"
                >
                  {emoji}
                </button>
              ))}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header Section */}
        <header className="px-6 py-5 border-b border-[#E2E5E9] flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/95">
          <div>
            <h2 className="text-xl font-bold text-[#1A1D21] flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-teal-400" />
              <span>Plugin System & Extensions</span>
            </h2>
            <p className="text-xs text-[#5F6B7A]">Install community widgets and diagrams scripts</p>
          </div>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-2xl border border-[#E2E5E9] bg-white/95 text-[#1A1D21] placeholder-slate-650 text-xs focus:outline-none focus:ring-1 focus:ring-teal-500"
            />
          </div>
        </header>

        {/* Tab switchers & Category selector */}
        <div className="px-6 py-3 border-b border-[#E2E5E9] flex gap-4 bg-white/95 text-xs font-semibold">
          <button 
            onClick={() => { setActiveTab('my-plugins'); setRunningPlugin(null); }}
            className={`pb-1.5 border-b-2 transition-all ${activeTab === 'my-plugins' ? 'border-teal-500 text-[#1A1D21]' : 'border-transparent text-[#5F6B7A] hover:text-slate-200'}`}
          >
            My Plugins ({installedPluginIds.length})
          </button>
          <button 
            onClick={() => { setActiveTab('marketplace'); setRunningPlugin(null); }}
            className={`pb-1.5 border-b-2 transition-all ${activeTab === 'marketplace' ? 'border-teal-500 text-[#1A1D21]' : 'border-transparent text-[#5F6B7A] hover:text-slate-200'}`}
          >
            Plugin Marketplace
          </button>
        </div>
        ...
        {activeTab === 'marketplace' && (
          <div className="px-6 py-2 border-b border-[#E2E5E9] bg-[#F8FAFB] overflow-x-auto flex gap-1.5 scrollbar-thin">
            {['All', 'Productivity', 'AI', 'Diagrams', 'Development', 'Utilities', 'Design', 'Charts', 'Export', 'Collaboration'].map(cat => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all ${
                  activeCategory === cat ? 'bg-teal-600 text-[#1A1D21]' : 'bg-[#E2E5E9] text-[#5F6B7A] hover:text-slate-200'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          >
            ...
            {runningPlugin ? (
              ...
              <div className="max-w-md mx-auto p-6 rounded-2xl bg-white/95 border border-[#E2E5E9] space-y-6">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{runningPlugin.icon}</span>
                    <div>
                      <h3 className="font-bold text-sm text-[#1A1D21]">{runningPlugin.name} Settings</h3>
                      <span className="text-[9px] text-[#9AA4AB]">v{runningPlugin.version} • By {runningPlugin.author}</span>
                    </div>
                  </div>
                  <button 
                    onClick={() => setRunningPlugin(null)}
                    className="text-xs text-[#9AA4AB] hover:text-slate-350"
                  >
                    Back
                  </button>
                </div>
              ...
              <div className="text-center py-20 text-[#9AA4AB] text-xs flex flex-col items-center justify-center border border-dashed border-[#E2E5E9] rounded-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCatalog.map((plugin) => {
                  ...
                  return (
                    <div
                      key={plugin.id}
                      className="p-5 rounded-2xl bg-white/95 border border-[#E2E5E9] hover:border-teal-500/30 transition-all flex flex-col justify-between h-40 shadow"
                    >
                      <div className="flex items-start gap-4">
                        <span className="text-3xl p-1 bg-[#F8FAFB] border border-[#E2E5E9] rounded-xl">{plugin.icon}</span>
                        <div className="min-w-0 flex-1 space-y-1">
                          <div className="flex justify-between items-center gap-2">
                            <h4 className="font-bold text-xs truncate text-slate-250">{plugin.name}</h4>
                            <span className="text-[8px] bg-[#F0F2F4] border border-[#E2E5E9] text-[#5F6B7A] px-1.5 py-0.5 rounded font-bold uppercase">
                              {plugin.category}
                            </span>
                          </div>
                      ...
                      {/* Actions footer */}
                      <div className="flex items-center justify-between border-t border-slate-850/60 pt-3 mt-2 text-[10px] text-[#9AA4AB]">
                          ...
                          {isInstalled ? (
                            <>
                              ...
                              <button
                                ...
                                className="px-2.5 py-1 bg-teal-600 hover:bg-teal-500 text-[#1A1D21] font-bold rounded flex items-center gap-1 transition-all"
                              >
                              ...
                              <button
                                ...
                                className="p-1 hover:bg-red-500/5 text-[#5F6B7A] hover:text-red-400 rounded transition-all"
                            ...
                            <button
                              ...
                              className="px-3 py-1 bg-[#E2E5E9] hover:bg-slate-700 text-[#5F6B7A] font-bold rounded flex items-center gap-1 transition-all"
                            >
        ...
        {/* Footer actions */}
        <footer className="px-6 py-4 border-t border-[#E2E5E9] bg-white/95 flex justify-between items-center text-[10px] text-[#9AA4AB]">
