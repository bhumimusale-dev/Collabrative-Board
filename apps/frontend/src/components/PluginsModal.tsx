        ...
        return (
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-2 bg-teal-650 hover:bg-teal-600 text-white rounded-xl text-xs font-bold transition-all"
            >
              Generate Diagram
            </button>
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header Section */}
        <header className="px-6 py-5 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0D0F12]/95">
          <div>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-2xl border border-slate-800 bg-[#0D0F12]/95 text-slate-200 placeholder-slate-650 text-xs focus:outline-none focus:ring-1 focus:ring-teal-500"
            />
          </div>
        </header>

        {/* Tab switchers & Category selector */}
        <div className="px-6 py-3 border-b border-slate-800 flex gap-4 bg-[#0D0F12]/95 text-xs font-semibold">
          <button 
            onClick={() => { setActiveTab('my-plugins'); setRunningPlugin(null); }}
            className={`pb-1.5 border-b-2 transition-all ${activeTab === 'my-plugins' ? 'border-teal-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
          >
            My Plugins ({installedPluginIds.length})
          </button>
          <button 
            onClick={() => { setActiveTab('marketplace'); setRunningPlugin(null); }}
            className={`pb-1.5 border-b-2 transition-all ${activeTab === 'marketplace' ? 'border-teal-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`}
          >
            Plugin Marketplace
          </button>
        </div>
        ...
        {activeTab === 'marketplace' && (
          <div className="px-6 py-2 border-b border-slate-850 bg-[#171B1F] overflow-x-auto flex gap-1.5 scrollbar-thin">
            {['All', 'Productivity', 'AI', 'Diagrams', 'Development', 'Utilities', 'Design', 'Charts', 'Export', 'Collaboration'].map(cat => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all ${
                  activeCategory === cat ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          >
            ...
            {runningPlugin ? (
              ...
              <div className="max-w-md mx-auto p-6 rounded-2xl bg-[#0D0F12]/95 border border-slate-800 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCatalog.map((plugin) => {
                  ...
                  return (
                    <div
                      key={plugin.id}
                      className="p-5 rounded-2xl bg-[#0D0F12]/95 border border-slate-850 hover:border-teal-500/30 transition-all flex flex-col justify-between h-40 shadow"
                    >
                          ...
                          {isInstalled ? (
                            <>
                              ...
                              <button
                                ...
                                className="px-2.5 py-1 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded flex items-center gap-1 transition-all"
                              >
        ...
        {/* Footer actions */}
        <footer className="px-6 py-4 border-t border-slate-800 bg-[#0D0F12]/95 flex justify-between items-center text-[10px] text-slate-500">
