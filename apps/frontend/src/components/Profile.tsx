...
export const Profile: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-white font-sans text-[#1A1D21] py-12 px-4">
      <div className="max-w-3xl mx-auto bg-[#F8FAFB] border border-[#E2E5E9] rounded-3xl overflow-hidden shadow-2xl">
        
        {/* Header Banner */}
        <div className="h-32 bg-gradient-to-r from-teal-600 to-purple-600 relative">
          <Link
            to="/dashboard"
            className="absolute top-4 left-4 px-4 py-2 bg-[#F8FAFB] hover:bg-[#F0F2F4] text-xs font-semibold text-[#1A1D21] rounded-full transition-all flex items-center gap-1"
          >
            ← Back to Dashboard
          </Link>
          <button
        ...
        <div className="px-8 pb-8 pt-0 relative">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 -mt-12 mb-8">
            <div className="w-24 h-24 rounded-full border-4 border-[#E2E5E9] overflow-hidden bg-[#E2E5E9] shadow-xl relative group">
              {user?.avatar ? (
                <img
                  src={user.avatar}
                  alt={user.name}
                ...
                <div className="w-full h-full bg-teal-600 flex items-center justify-center text-3xl font-bold text-[#1A1D21] uppercase">
                  {user?.name.charAt(0)}
                </div>
              )}
              ...
              <label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-all">
                <span className="text-[10px] font-bold text-[#1A1D21] uppercase tracking-wider text-center px-1">
            ...
            <div className="text-center sm:text-left flex-1">
              ...
              <p className="text-sm text-[#5F6B7A]">@{user?.username}</p>
          ...
          <hr className="border-[#E2E5E9] mb-8" />
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl border border-[#E2E5E9] bg-white text-[#1A1D21] placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all text-sm resize-none"
              />
              {errors.bio && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div>
                ...
                <select
                  {...register('theme')}
                  className="w-full px-4 py-3 rounded-xl border border-[#E2E5E9] bg-white text-[#1A1D21] focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all text-sm"
                >
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-3 rounded-xl bg-teal-600 hover:bg-teal-700 text-[#1A1D21] font-semibold text-sm shadow-md transition-all disabled:opacity-50 flex items-center gap-2"
              >
